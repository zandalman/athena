...
void calcAffineModel(Real time, const affineModel &am, affineModel &amdot) {
  ...
  amdot.Hdot = 2.0*M_PI * tde->rho0 
    * (pow(am.L * am.Dlt * am.H, -tde->gm1) / am.H - 2.0 / (am.L * (am.Dlt + am.H)))
    -1.0/(am.r*am.r*am.r) * am.H;
  amdot.Dlt = am.Dltdot;
  amdot.Dltdot = 2.0*M_PI * tde->rho0
    * (pow(am.L * am.Dlt * am.H, -tde->gm1) / am.Dlt - 2.0 / (am.L * (am.Dlt + am.H)))
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for ( int i=1; i<tde->num_time; i++ ) {
    ...
    outputFile << time << ","
               ...
               << am.Dlt << "," << am.Dltdot << "," << am.vpar << std::endl;
